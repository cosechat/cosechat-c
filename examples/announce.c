...
int main(void) {
  ...
  ret = cc_announce_build(&key, "Alice", 5, NULL, 0, ann_pkt, sizeof(ann_pkt),
                          &ann_len, &rng);
  ...
  ret = cc_presence_build(&key, "Alice", 5, pres_pkt, sizeof(pres_pkt),
                          &pres_len, &rng);
  ...
  printf("presence parse: %s, name='%.*s', addr_match=%s\n",
         ret == CC_OK ? "ok" : "FAILED", (int)pres.name_len, pres.name,
